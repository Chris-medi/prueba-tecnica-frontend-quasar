<template>
  <q-layout v-if="fileStore.pin_files.length > 0" class="flex column full-width text-center"
    style="min-height: 0;">
    <h4 class="q-ma-md">Archivos anclados</h4>
    <section class="full-width flex row wrap items-start q-px-md">
    <CardFile v-for="objt in fileStore.pin_files" v-bind:key="objt.id" v-bind:show="false" :show_pin="false" v-bind:objt="objt"></CardFile>
    </section >
  </q-layout>
  <q-layout class="flex column full-width text-center"
    ...
    <section class="full-width flex row wrap items-start q-px-md">
    <CardFile v-for="objt in fileStore.files" v-bind:key="objt.id" v-bind:objt="objt" :show_pin="true"></CardFile>
    </section >
  </q-layout>
</template>
